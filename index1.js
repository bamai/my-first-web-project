...
function createMovieList(element) {
    ...
    let movieCompare = function(movie1,movie2){
        let title1 = movie1.slice(0,movie1.lastIndexOf(",")).toLowerCase();
        let title2 = movie2.slice(0,movie2.lastIndexOf(",")).toLowerCase();
        let year1 = movie1.slice(movie1.lastIndexOf(","),movie1.length);
        let year2 = movie2.slice(movie2.lastIndexOf(","),movie2.length);
        console.log(title1,title2);
        let equalYears = (year1[0]!==","||year2[0]!==","||year1===year2);
        console.log(equalYears);
        return title1===title2&&equalYears;
    }
    ...
    let deleteFromDropdown = function(movie){
        for(let i=0;i<dropdown.children.length;i++){
            ...
            title = title.slice(0,title.lastIndexOf(',')).toLowerCase();
            if(title === movie){
    ...
    let dropdownHandler = function(database){
        ...
        for(let i=0;i<dropdownItems.length;i++){
            ...
            for(let j=0;j<database.length;j++){
                if(database[j].Title === dropdownItems[i]){
                    exists = true;
                    break;
                }
            }
        ...
        for(let i=0; i<database.length;i++){
            let name = database[i].Title.toLowerCase();
            if(!dropdownItems.includes(name)){
